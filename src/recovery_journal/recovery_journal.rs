...
impl RecoveryJournal {
    pub fn parse(bytes: &[u8]) -> Result<Self, String> {
        let mut i: usize = 0;
        if bytes.len() < i + 3 {
            return Err("Not enough data for journal header".to_string());
        }
        ...
        let system_journal = if y_flag {
            ...
            if bytes.len() < i + 2 {
                return Err("Not enough data for system journal header".to_string());
            }
            ...
            if bytes.len() < i + length {
                return Err("Not enough data for system journal chapters".to_string());
            }
        ...
        for _channel_index in 0..total_channels {
            if bytes.len() < i + 4 {
                return Err("Not enough data for channel journal header".to_string());
            }
            ...
            if bytes.len() < i + length as usize {
                return Err("Not enough data for channel journal chapters".to_string());
            }
